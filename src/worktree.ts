...
export interface PrBodyContext {
  instanceName: string;
  skills: Array<{ name: string; status: string; durationSec?: number; issueCount?: number }>;
  pipelineDurationSec?: number;
  costUsd: number;
  todoTitle?: string;
  oracleDecisions?: Array<{ question: string; chosen: string; confidence: number }>;
  testsPassed?: boolean;
  testDurationSec?: number;
  issues?: Array<{ description: string; status: string }>;
}

/** Maximum PR body size (GitHub limit is 65536, we use 60000 for safety). */
const PR_BODY_MAX_CHARS = 60_000;
const PR_BODY_TRUNCATION_MARKER = "\n\n... truncated (exceeded GitHub body limit)";
...
export function buildPrBody(context: PrBodyContext): string {
  ...
  lines.push("## GaryClaw Autonomous Job Summary");
  lines.push("");
  lines.push(`**Instance:** ${context.instanceName}`);

  const skillNames = context.skills.map((s) => s.name);
  lines.push(`**Pipeline:** ${skillNames.join(" → ")}`);

  if (context.pipelineDurationSec !== undefined) {
    lines.push(`**Duration:** ${formatDurationForPr(context.pipelineDurationSec)}`);
  }
  lines.push(`**Cost:** $${context.costUsd.toFixed(3)}`);
  lines.push("");

  // Skills table
  lines.push("### Skills Run");
  lines.push("| Skill | Status | Duration | Issues |");
  lines.push("|-------|--------|----------|--------|");
  for (const skill of context.skills) {
    const duration = skill.durationSec !== undefined ? formatDurationForPr(skill.durationSec) : "—";
    const issues = skill.issueCount !== undefined ? String(skill.issueCount) : "—";
    lines.push(`| ${skill.name} | ${skill.status} | ${duration} | ${issues} |`);
  }
  lines.push("");

  // TODO item
  if (context.todoTitle) {
    lines.push("### TODO Item");
    lines.push(`**${context.todoTitle}**`);
    lines.push("");
  }

  // Oracle decisions (cap at 10)
  if (context.oracleDecisions && context.oracleDecisions.length > 0) {
    lines.push("### Oracle Decisions");
    const decisions = context.oracleDecisions.slice(0, 10);
    for (const d of decisions) {
      lines.push(`- "${d.question}" → ${d.chosen} (confidence: ${d.confidence})`);
    }
    if (context.oracleDecisions.length > 10) {
      lines.push(`- ... and ${context.oracleDecisions.length - 10} more decisions`);
    }
    lines.push("");
  }
  ...
  if (context.testsPassed !== undefined) {
    ...
    const status = context.testsPassed ? "PASSED" : "FAILED";
    const duration = context.testDurationSec !== undefined ? ` (${context.testDurationSec.toFixed(1)}s)` : "";
    lines.push(`Pre-merge tests: ${status}${duration}`);
    lines.push("");
  }

  // Issues found
  if (context.issues && context.issues.length > 0) {
    lines.push("### Issues Found & Fixed");
    const capped = context.issues.slice(0, 20);
    for (const issue of capped) {
      const tag = issue.status === "fixed" ? "FIXED" : issue.status.toUpperCase();
      lines.push(`- [${tag}] ${issue.description}`);
    }
    if (context.issues.length > 20) {
      lines.push(`- ... and ${context.issues.length - 20} more issues`);
    }
    lines.push("");
  }

  lines.push("---");
  lines.push("*Generated by [GaryClaw](https://github.com/0raclide/GaryClaw) daemon*");

  let body = lines.join("\n");

  // Hard cap at PR_BODY_MAX_CHARS
  if (body.length > PR_BODY_MAX_CHARS) {
    body = body.slice(0, PR_BODY_MAX_CHARS - PR_BODY_TRUNCATION_MARKER.length) + PR_BODY_TRUNCATION_MARKER;
  }

  return body;
}

/** Format seconds into human-readable duration for PR body. */
function formatDurationForPr(seconds: number): string {
 ...
 * Returns true if gh is usable, false otherwise.
 */
export function isGhAvailable(repoDir: string): boolean {
  try {
 ...
 *
 * Error handling:
 * - gh not available: returns { created: false, reason }
 * - push fails with existing PR: attempts to update existing PR body
 * - auto-merge not enabled on repo: logs warning, PR stays open
 */
export function createPullRequest(
  ...
  const warn = resolveWarnFn(options.onWarn);
  const branch = branchName(instanceName);

  // 1. Check gh is available
  if (!isGhAvailable(repoDir)) {
    return { created: false, reason: "gh CLI not available or not authenticated" };
  }

  // 2. Push branch to remote
  try {
    execFileSync("git", ["push", "origin", branch, "--force-with-lease"], {
      cwd: repoDir,
      stdio: "pipe",
      timeout: 60_000,
    });
    ...
    return { created: false, reason: `Push failed: ${msg}` };
  }
  ...
  try {
    const prList = execFileSync(
      ...
      { cwd: repoDir, stdio: "pipe", encoding: "utf-8", timeout: 15_000 },
    ).trim();
    const parsed = JSON.parse(prList);
    if (Array.isArray(parsed) && parsed.length > 0 && typeof parsed[0].number === "number") {
      existingPrNumber = parsed[0].number;
    }
  } catch {
    // Ignore — will try to create a new PR
  }
  ...
  if (existingPrNumber !== null) {
    ...
    try {
      execFileSync(
        "gh",
        ["pr", "edit", String(existingPrNumber), "--title", options.title, "--body", options.body],
        { cwd: repoDir, stdio: "pipe", timeout: 15_000 },
      );
      // Get PR URL
      const viewOutput = execFileSync(
        "gh",
        ["pr", "view", String(existingPrNumber), "--json", "url"],
        { cwd: repoDir, stdio: "pipe", encoding: "utf-8", timeout: 15_000 },
      ).trim();
      prUrl = JSON.parse(viewOutput).url ?? `#${existingPrNumber}`;
      prNumber = existingPrNumber;
      warn(`[worktree-pr] Updated existing PR #${prNumber}`);
    } catch (editErr) {
      const msg = editErr instanceof Error ? editErr.message : String(editErr);
      return { created: false, reason: `Failed to update existing PR #${existingPrNumber}: ${msg}` };
    }
  } else {
    // Create new PR
    const args = [
  ...
  // 5. Enable auto-merge if requested (best-effort)
  ...
  if (options.autoMerge !== false) {
    ...
    try {
      execFileSync(
        "gh",
        ["pr", "merge", String(prNumber), "--auto", `--${mergeMethod}`],
        { cwd: repoDir, stdio: "pipe", timeout: 15_000 },
      );
      autoMergeEnabled = true;
    } catch {
      // Auto-merge may not be enabled on the repo — this is non-fatal
      warn(`[worktree-pr] Auto-merge not enabled for PR #${prNumber} (repo setting may be disabled)`);
    }
  }
