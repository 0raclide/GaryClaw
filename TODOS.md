...
## P3: Post-Merge Test Verification + Auto-Revert Safety Net

**What:** After every auto-merge to main, run the full test suite on the merged result. If tests fail, auto-revert the merge commit and create a high-priority bug TODO in TODOS.md with the failure details. Notify via macOS notification. Track revert count in the dashboard.

**Why:** The daemon auto-merges worktree branches to main after pipeline completion. If a merge introduces a regression (merge conflict resolution error, test interaction between parallel branches, or a bug the QA skill missed), main stays broken until a human notices. Every subsequent pipeline cycle starts from a broken base, compounding the damage. This is the #1 safety gap in autonomous overnight operation — the daemon has no feedback loop between "merged" and "main is healthy."

**Implementation:**
- New function `verifyMergeHealth()` in `worktree.ts`: after successful `mergeWorktreeBranch()`, run `npm test` on the base repo. If exit code ≠ 0, `git revert HEAD --no-edit`, write failure details to `.garyclaw/merge-reverts.jsonl`.
- Wire into `job-runner.ts` post-merge path: call `verifyMergeHealth()` after merge succeeds. On revert, set job status to `"merge_reverted"`, create a P2 bug TODO with test failure output, send notification.
- Extend `failure-taxonomy.ts` with `"test-regression"` category.
- Extend dashboard with merge revert count and revert rate.
- Guard: skip verification if `config.merge.skipTestVerification` is true (for repos with slow test suites).

**Effort:** S (human: ~3 days / CC: ~25 min)
**Depends on:** Nothing
**Added by:** Auto-generated by prioritize on 2026-03-30 (backlog exhausted)

## P3: Daemon Continuous Self-Improvement Mode
