...
**The meta-story:** 740 commits. 43 source modules. The daemon built 22 of them itself — it designed its own oracle, built its own skill selection, QA'd its own code, and invented new features when the backlog ran dry. 96% of commits are daemon-generated. This isn't a tool that runs tasks — it's a system that evolves.
...
- 43 source modules, 206 test files, 3341 tests
...
- **Oracle decision cache** — `OracleCache` in `oracle-cache.ts` caches repeated Oracle decisions at zero cost. `normalizeQuestion()` strips variable tokens (file paths with 2+ segments, numbers, timestamps, long quoted strings) and reduces to a sorted keyword bag. `computeCacheKey()` concatenates normalized question + sorted option labels. `minHits` threshold (default: 5) required before promoting an answer to cache. `warmFromOutcomes()` pre-populates from decision-outcomes.md at startup (question-only keys, options verified at lookup). Cache lookup falls back from exact key to question-only key. `invalidate()` clears both exact and question-only keys. Wired into ask-handler.ts (partial-batch: cached questions resolved inline, uncached sent to Oracle batch/serial, answers recorded for future hits). Orchestrator initializes per-skill, warms from `readDecisionOutcomes()`, passes to ask-handler, invalidates on reflection failure outcomes. `onCacheEvent` callback emits `oracle_cache_hit`/`oracle_cache_miss`/`oracle_cache_invalidated` events. Daemon config validates `oracleCache.enabled` + `oracleCache.minHits`.
...
| `test/prioritize.test.ts` | 81 | parseTodoItems, loadOvernightGoal, loadOracleContext, formatPipelineContext, buildPrioritizePrompt, aggregateFailurePatterns, getDecisionQualityTrends, measureRecentImpact, per-category stats injection, Wow factor weights |
...
| `test/project-type-wiring.test.ts` | 12 | Doctor check #10 (stale project type cache), buildEvaluatePrompt/buildImplementPrompt/buildPrioritizePrompt project type injection, bootstrap saveProjectType integration |
